...
def test_agent_spawning_e2e(live_server):
    ...
    host_sio.connect(
        live_server, namespaces=["/terminal"], headers={"x-host-token": "spawn-token-2"}
    )

    # 3. UI triggers spawn via REST API
    r_spawn = httpx.post(
        f"{live_server}/agents/spawn", json={"host_id": host_id, "tool_name": "gemini"}
    )
...
def test_http_endpoints(live_server):
    ...
    # Register a host
    r = httpx.post(
        f"{live_server}/hosts", json={"name": "test-host", "host_token": "secret"}
    )
    assert r.status_code == 200
    assert "id" in r.json()
...
def test_socketio_connection(live_server):
    ...
    httpx.post(
        f"{live_server}/hosts", json={"name": "spawn-host", "host_token": "spawn-token"}
    )

    sio.connect(
        live_server, namespaces=["/terminal"], headers={"x-host-token": "spawn-token"}
    )
    time.sleep(0.5)
...
def test_socketio_relay_multiplex_e2e(live_server):
    ...
    r_host = httpx.post(
        f"{live_server}/hosts", json={"name": "relay-host", "host_token": "relay-token"}
    )
    ...
    host_sio.connect(
        live_server, namespaces=["/terminal"], headers={"x-host-token": "relay-token"}
    )
    ...
    test_telemetry = {
        "projects_root": "/test",
        "available_projects": ["proj1", "proj2"],
    }
    ...
    host_sio.emit(
        "terminal_output",
        {"sid": agent_id, "output": test_output},
        namespace="/terminal",
    )
    ...
    ui_sio.emit(
        "terminal_input",
        {"target_sid": agent_id, "input": test_input},
        namespace="/terminal",
    )
    ...
    for _ in range(10):
        if any(
            d.get("target_sid") == agent_id and d.get("input") == test_input
            for d in host_received_input
        ):
            break
        time.sleep(0.1)
    ...
    matching_update = next(
        d for d in ui_received_tel_updates if d.get("agent_id") == agent_id
    )
